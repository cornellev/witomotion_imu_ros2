...
ROSWitmotionSensorController::ROSWitmotionSensorController()
    ...
    _restart_service_name =
        node->get_parameter("restart_service_name").get_parameter_value().get<std::string>();

    restart_service = node->create_service<std_srvs::srv::Empty>(_restart_service_name,
        &ROSWitmotionSensorController::Restart);

    /* IMU */
    node->declare_parameter("imu_publisher.topic_name", "imu");
    _imu_topic =
        node->get_parameter("imu_publisher.topic_name").get_parameter_value().get<std::string>();

    imu_publisher = node->create_publisher<sensor_msgs::msg::Imu>(_imu_topic, 10);

    node->declare_parameter("imu_publisher.frame_id", "imu");
    imu_frame_id =
        node->get_parameter("imu_publisher.frame_id").get_parameter_value().get<std::string>();

    node->declare_parameter("imu_publisher.use_native_orientation", false);
    imu_native_orientation = node->get_parameter("imu_publisher.use_native_orientation")
                                 .get_parameter_value()
                                 .get<bool>();

    node->declare_parameter("imu_publisher.measurements.acceleration.enabled", false);
    imu_enable_accel = node->get_parameter("imu_publisher.measurements.acceleration.enabled")
                           .get_parameter_value()
                           .get<bool>();

    load_parameter(imu_enable_accel, "imu_publisher.measurements.acceleration.covariance", -1.0,
    ...
    load_parameter(imu_enable_velocities, "imu_publisher.measurements.angular_velocity.covariance",
        -1.0, imu_velocity_covariance);

    node->declare_parameter("imu_publisher.measurements.orientation.enabled", false);
    imu_enable_orientation = node->get_parameter("imu_publisher.measurements.orientation.enabled")
                                 .get_parameter_value()
                                 .get<bool>();

    load_parameter(imu_enable_orientation, "imu_publisher.measurements.orientation.covariance",
        -1.0, imu_orientation_covariance);

    /* TEMPERATURE */
    node->declare_parameter("temperature_publisher.enabled", false);
    temp_enable =
        node->get_parameter("temperature_publisher.enabled").get_parameter_value().get<bool>();
    ...
    if (temp_enable) {
        ...
        std::transform(temp_from_str.begin(), temp_from_str.end(), temp_from_str.begin(),
            ::toupper);
    ...
    magnetometer_enable =
        node->get_parameter("magnetometer_publisher.enabled").get_parameter_value().get<bool>();
    if (magnetometer_enable) {
        node->declare_parameter("magnetometer_publisher.topic_name", "mag");
        _magnetometer_topic = node->get_parameter("magnetometer_publisher.topic_name")
                                  .get_parameter_value()
                                  .get<std::string>();

        node->declare_parameter("magnetometer_publisher.frame_id", "mag_frame");
        magnetometer_frame_id = node->get_parameter("magnetometer_publisher.frame_id")
                                    .get_parameter_value()
                                    .get<std::string>();

        load_parameter(magnetometer_enable, "magnetometer_publisher.covariance", 0,
        ...
        magnetometer_publisher =
            node->create_publisher<sensor_msgs::msg::MagneticField>(_magnetometer_topic, 1);
    }
    /* BAROMETER */
    node->declare_parameter("barometer_publisher.enabled", false);
    barometer_enable =
        node->get_parameter("barometer_publisher.enabled").get_parameter_value().get<bool>();
    if (barometer_enable) {
        ...
        barometer_publisher =
            node->create_publisher<sensor_msgs::msg::FluidPressure>(_barometer_topic, 1);
    }
    /* ALTIMETER */
    node->declare_parameter("altimeter_publisher.enabled", false);
    altimeter_enable =
        node->get_parameter("altimeter_publisher.enabled").get_parameter_value().get<bool>();
    if (barometer_enable) {
    ...
    orientation_enable =
        node->get_parameter("orientation_publisher.enabled").get_parameter_value().get<bool>();
    ...
    if (orientation_enable) {
        ...
        orientation_publisher =
            node->create_publisher<geometry_msgs::msg::Quaternion>(_orientation_topic, 1);
    }
    /* GPS */
    ...
    if (gps_enable) {
        ...
        _ground_speed_topic = node->get_parameter("gps_publisher.ground_speed_topic_name")
                                  .get_parameter_value()
                                  .get<std::string>();

        ground_speed_publisher =
            node->create_publisher<geometry_msgs::msg::Twist>(_ground_speed_topic, 1);

        node->declare_parameter("gps_publisher.navsat_variance_topic_name", "accuracy");
        _accuracy_topic = node->get_parameter("gps_publisher.navsat_variance_topic_name")
                              .get_parameter_value()
                              .get<std::string>();

        accuracy_publisher = node->create_publisher<geometry_msgs::msg::Vector3>(_accuracy_topic,
            1);

        node->declare_parameter("gps_publisher.navsat_satellites_topic_name", "satellites");
        _satellites_topic = node->get_parameter("gps_publisher.navsat_satellites_topic_name")
                                .get_parameter_value()
                                .get<std::string>();
        ...
        _gps_altitude_topic = node->get_parameter("gps_publisher.navsat_altitude_topic_name")
                                  .get_parameter_value()
                                  .get<std::string>();

        gps_altitude_publisher = node->create_publisher<std_msgs::msg::Float32>(_gps_altitude_topic,
            1);
    }
    /* REALTIME CLOCK */
    ...
    if (rtc_enable) {
        ...
        rtc_presync =
            node->get_parameter("rtc_publisher.presync").get_parameter_value().get<bool>();
    }

    /*Initializing QT fields*/
    node->declare_parameter("port", "witmotion");
    // port_name = node->get_parameter("port").get_parameter_value().get<std::string>();
    port_name = "/dev/witmotion";
    ...
    int_timeout_ms = node->get_parameter("timeout_ms").get_parameter_value().get<int>();
    ...
    RCLCPP_INFO(node->get_logger(), "Starting node with lib version (%s).",
        witmotion::library_version().c_str());
    reader_thread.start();
}
...
bool ROSWitmotionSensorController::Restart(std::shared_ptr<std_srvs::srv::Empty::Request> request,
    ...
    if (!suspended) {
        ...
        RCLCPP_WARN_STREAM_SKIPFIRST(rclcpp::get_logger("ROSWitmotionSensorController"),
...
void ROSWitmotionSensorController::imu_process(const witmotion_datapacket& packet) {
    if (!(imu_enable_accel || imu_enable_velocities || imu_enable_orientation)) return;
    ...
    if ((imu_enable_accel == imu_have_accel) && (imu_enable_velocities == imu_have_velocities)
...
void ROSWitmotionSensorController::magnetometer_process(const witmotion_datapacket& packet) {
    if (!magnetometer_enable) return;
    static sensor_msgs::msg::MagneticField msg;
    static float x, y, z, t;
    msg.header.frame_id = magnetometer_frame_id;
    msg.header.stamp = rclcpp::Clock().now();
    for (size_t i = 0; i < 9; i++) msg.magnetic_field_covariance[i] = magnetometer_covariance[i];
    decode_magnetometer(packet, x, y, z, t);
...
void ROSWitmotionSensorController::altimeter_process(const witmotion_datapacket& packet) {
    ...
    if (altimeter_enable) {
        ...
        if (!have_altitude) have_altitude = true;
...
void ROSWitmotionSensorController::gps_process(const witmotion_datapacket& packet) {
    if (gps_enable) {
        ...
        msg.position_covariance_type =
            have_accuracy ? sensor_msgs::msg::NavSatFix::COVARIANCE_TYPE_DIAGONAL_KNOWN
...
void ROSWitmotionSensorController::rtc_process(const witmotion_datapacket& packet) {
    if (rtc_enable) {
        ...
        decode_realtime_clock(packet, year, month, day, hour, minute, second, millisecond);
        QDateTime qt_time;
...
void ROSWitmotionSensorController::Error(const QString& description) {
    RCLCPP_ERROR(rclcpp::get_logger("ROSWitmotionSensorController"), "Sensor error: %s",
...
void ROSWitmotionSensorController::load_parameter_d(std::string param_name, double init_val,
    double& param_var) {
    try {
        node->declare_parameter(param_name, init_val);
        param_var = node->get_parameter(param_name).get_parameter_value().get<double>();
    } catch (const rclcpp::exceptions::InvalidParameterTypeException& ex) {
        RCLCPP_WARN(node->get_logger(), "Exception reading param (%s).\nReading as string vector",
            ex.what());
...
void ROSWitmotionSensorController::load_parameter_f(std::string param_name, float init_val,
    float& param_var) {
    try {
        node->declare_parameter(param_name, init_val);
        param_var = node->get_parameter(param_name).get_parameter_value().get<float>();
    } catch (const rclcpp::exceptions::InvalidParameterTypeException& ex) {
        RCLCPP_WARN(node->get_logger(), "Exception reading param (%s).\nReading as string vector",
            ex.what());
        node->declare_parameter(param_name, std::to_string(init_val));
...
void ROSWitmotionSensorController::load_parameter(bool is_active, std::string param_name,
    double first_val, std::vector<double>& param_vector) {
    if (is_active) {
        try {
            node->declare_parameter(param_name,
                std::vector<double>({first_val, 0, 0, 0, 0, 0, 0, 0, 0}));
            param_vector =
                node->get_parameter(param_name).get_parameter_value().get<std::vector<double>>();
        } catch (const rclcpp::exceptions::InvalidParameterTypeException& ex) {
            RCLCPP_WARN(node->get_logger(),
                "Exception reading param (%s).\nReading as string vector", ex.what());
            node->declare_parameter(param_name, std::vector<std::string>({std::to_string(first_val),
                                                    "0", "0", "0", "0", "0", "0", "0", "0"}));
            auto tmp = node->get_parameter(param_name)
                           .get_parameter_value()
